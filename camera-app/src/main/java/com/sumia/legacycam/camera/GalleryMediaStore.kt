...
object GalleryMediaStore {
    ...
    suspend fun loadAllMedia(context: Context): List<GalleryItemPayload> = withContext(Dispatchers.IO) {
        queryMedia(context).map { entry ->
            GalleryItemPayload(
    ...
    private fun findMediaById(context: Context, mediaId: String): MediaEntry? {
        return queryMedia(context).firstOrNull { it.mediaId == mediaId }
    }
