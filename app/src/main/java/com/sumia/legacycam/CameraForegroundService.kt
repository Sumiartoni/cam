...
class CameraForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_STOP -> {
                CameraStreamingController.stop("Mode kamera dihentikan dari foreground service.")
    ...
    private fun buildNotification(state: CameraServiceState): Notification {
        ...
        val title = if (state.isRunning) {
            "LegacyCam kamera aktif"
        } else {
            "LegacyCam kamera standby"
        }

        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_legacycam)
    ...
    private fun createNotificationChannel() {
        ...
        val channel = NotificationChannel(
            ...
            description = "Menjaga mode kamera LegacyCam tetap aktif di background."
        }
